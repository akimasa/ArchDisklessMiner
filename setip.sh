...
if [ "$NFSBOOT" == "1" ]; then
    cp nfsboot /srv/arch/usr/lib/initcpio/hooks/net_nfs4
    cp install-net_nfs4.nfsboot /srv/arch/usr/lib/initcpio/install/net_nfs4
fi
...
# generate initramfs
if [ ! "$SKIPINITCPIO" == "1" ]; then eval $CHROOT /srv/arch/ mkinitcpio -k /boot/vmlinuz-linux -g /boot/initramfs-linux-fallback.img; fi
